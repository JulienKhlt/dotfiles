...
module completions {
  ...
  def get-groups []: nothing -> list<string> {
    ...
    try { open $file | get dependency-groups | columns } catch { [] }
  }
  ...
  def get-main-packages [] {
    let file = (find-pyproject-file)
    try { open $file | get project.dependencies | parse-package-names } catch { [] }
  }

  # Get packages from "project.optional-dependencies"
  def get-optional-packages [] {
    let file = (find-pyproject-file)
    try { open $file | get project.optional-dependencies | parse-package-names } catch { [] }
  }
  ...
  def get-dependency-group-packages [only_group?: string] {
    let file = (find-pyproject-file)
    let dg = try { open $file | get dependency-groups } catch { [] }
  ...
  export def "nu-complete uv packages" [context: string, position?:int] {
    ...
    let go = $prev_tokens | enumerate | find '--group' | get index.0
    let group = if ($go | is-not-empty) { $prev_tokens | get ($go + 1)}
    if ($group | is-empty ) {
